...
static const int COINBASE_MATURITY_OREX = 72;
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const int STAKE_TARGET_SPACING = 5 * 60; // block spacing 
static const int STAKE_MIN_AGE = 60 * 60 * 24 * 3; // minimum age for coin age
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 10; // stake age of full weight
static const int64 COIN_PREMINE = 32000000 * COIN;
static const int64 COIN_YEAR_REWARD = 1 * CENT;
static const int64 MAX_MINT_PROOF_OF_WORK = 0.3 * COIN;
static const int64 MAX_POS_REWARD = 0.3 * COIN;
static const int64 LAST_POW_BLOCK = 2880; // Block height of the last proof of work block
static const int DAILY_BLOCKCOUNT = 288;
static const int YEARLY_BLOCKCOUNT = 105120;
...
static const uint256 hashGenesisBlockOfficial("0x00000034749a11ccf915f6d643e5afe371fc5d4ef3d068b872168189a185f9a4");
static const uint256 hashGenesisBlockTestNet("0x00003bf92b725a888c837bf24cef8370b145f444420766398471c7f005dade7a");
...
public:
    ...
    {
        // orexcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // orexcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // orexcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // orexcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // orexcoin: whether transaction is a coinstake
    bool fCoinStake;

    // orexcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // orexcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // orexcoin
            undo.nTime = nTime;            // orexcoin
        }
        return true;
    }
...
public:
    ...
    // orexcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // orexcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // orexcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // orexcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // orexcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (orexcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // orexcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // orexcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // orexcoin: block index flags
    enum
    {
