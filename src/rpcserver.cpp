...
// orexcoin: remove transaction from mempool
...
// orexcoin: reserve balance from being staked for network protection
...
// orexcoin: check wallet integrity
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Orexcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Orexcoin server stopping";
}
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use orexcoind";
