...
static unsigned int nStakeSplitAge = (60 * 60 * 24 * 10);
...
// orexcoin: optional setting to unlock wallet for block minting only;
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //orexcoin: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
...
// orexcoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // orexcoin: timestamp must not exceed spend time
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            ploop
            {
                ...
                // orexcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // orexcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// orexcoin: create coin stake transaction
...
// orexcoin: check 'spent' consistency between wallet and txindex
// orexcoin: fix wallet spent state according to txindex
...
// orexcoin: disable transaction (only for coinstake)
